...
class Conv_BaseBlock(nn.Module):
    """
    ResNet-basicblock-like conv module
    """
    def __init__(
        self,
        embed_dim: int,
        ...
        super().__init__()
        self.conv1 = nn.Conv2d(embed_dim, mid_dim, kernel_size=3, padding=1, bias=bias)
        self.bn1 = norm(num_features=mid_dim)
        self.activation = layers.get_activation_fn(activation)
        self.conv2 = nn.Conv2d(mid_dim, embed_dim, kernel_size=3, padding=1, bias=bias)
        self._reset_parameters()
