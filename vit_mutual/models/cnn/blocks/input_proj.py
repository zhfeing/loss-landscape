...
class ResnetInputProj_v2(nn.Module):
    def __init__(
        self,
        image_channels: int,
        embed_dim: int,
...
class ResnetInputProj(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conv1 = nn.Conv2d(image_channels, embed_dim, kernel_size=7, stride=2, padding=3, bias=False)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
__REGISTERED_INPUT_PROJ__ = {
    ...
    "resnet_like_v2": ResnetInputProj_v2,
}
