...
def mutual_worker(
    ...
    for name, model in mutual_model.models.items():
        logger.info(
            "Built model %s with %d parameters, %d trainable parameters",
            name,
            utils.count_parameters(model, include_no_grad=True),
            utils.count_parameters(model, include_no_grad=False)
        )
    logger.info("Total %d parameters", utils.count_parameters(mutual_model))
